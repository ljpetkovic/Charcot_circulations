...
output_file_path = "/Users/ljudmilapetkovic/Library/CloudStorage/Dropbox/SU/ObTIC/CHARCOT/Charcot_circulations/csv/output_autres_echantillon_130426.csv"
...
for scores in regex_scores.values():
    scores["Normalized TF-IDF"] = (
        (scores["TF-IDF"] - min_tfidf) / (max_tfidf - min_tfidf)
        if max_tfidf > min_tfidf else 0
    )
    scores["Normalized BM25"] = (
        (scores["BM25"] - min_bm25) / (max_bm25 - min_bm25)
